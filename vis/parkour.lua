...
local sexps, pb, sb, se, eb, ee, wb, we, pWB, WB, WE, ub, ue
...
local function insert_at(prefix_pos, pos, prefix_list, list)
	...
	if prefix_pos < pos then
		...
		if not prefix_list[0] then prefix_list[0] = true end
		prefix_list[pos] = prefix_pos
	end
	return pos
end
...
local function get_horizons(text)  -- luacheck: ignore
	local horizons = {}
	local pattern = l.P{p(horizons) * l.P{l.Ct(Op^0 * d1 * ((C * (space^0 * C)^0 + Str + atom) + 1 - Op^0 * d1 - d2 + l.V(1))^0 * d2)} + (p(horizons) * ((C * (space^0 * C)^0 + Str) + atom)) + (d1 + D2) + l.P"\f" * l.P"\n"^1 + space^1}
...
local function get_sexps(win, pos, advance)
	local base = first_invalid and before(pb, first_invalid + 2)
	if not base then
		sexps, pb, sb, se, eb, ee, wb, we, pWB, WB, WE, ub, ue = {}, {}, {}, {}, {}, {}, {}, {}, {}, {}, {}, {}, {}
	elseif first_invalid + 1 >= base then
		...
		if #ue > 0 then
			local nearest_ue = after(ue, pos)
			if nearest_ue then pos = nearest_ue - 1 end
		end
		...
		for _, list in ipairs({pb, sb, se, eb, ee, wb, we, pWB, WB, WE, ub, ue}) do
			take_while(function(e) return e < base end, list)
		end
	end
	local pattern = l.P{p(pb) * pt(sexps, l.P{l.Ct(Pm(pWB, WB) * d1 * (p(wb) * (p(eb) * (C * (space^0 * C)^0 + Str) * p1(ee) + atom) * p1(we) + 1 - Op^0 * d1 - d2 + l.V(1))^0 * P(WE) * d2)}) + (p(pb) * p(wb) * (p(eb) * (C * (space^0 * C)^0 + Str) * p1(ee) + atom) * p1(we)) + (p(ub) * p(WB) * d1 + p(ue) * p(WE) * D2) + (p(se) * l.P"\f" * l.P"\n"^1 * p1(sb)) + space^1}
	content = win.file:content(0, win.file.size)
	if advance then
		first_invalid = l.P(pattern^-(advance + 2)):match(content, base) - 1
	else
		first_invalid = l.P((l.Cmt(l.Cc(pos + 1), past) * pattern)^0):match(content, base) - 1
	end
...
function H.M.prev_sexp(win, pos, estart, efinish)
	local prev_closing = H.M.prev_closing(win, pos)
	local prev_end = H.M.prev_end(win, pos, estart, efinish)
...
function H.M.next_sexp(win, pos, estart, efinish)
	local next_opening = H.M.next_opening(win, pos)
	local next_begin = H.M.next_begin(win, pos, estart, efinish)
...
function H.M.next_paragraph(win, pos)
	local newpos = after(pb, pos + 1)
	return newpos and newpos - 1 or (first_invalid == win.file.size and first_invalid) or pos
end
...
function H.M.next_section(win, pos)
	local newpos = after(sb, pos + 1)
	return newpos or (first_invalid == win.file.size and first_invalid) or pos
end
...
function H.T.outer_string(win, pos)
	...
	if start and '"' == content:sub(start + 1, start + 1) and '"' == content:sub(finish, finish) then
		return start, finish
	end
end
...
function H.T.outer_comment(win, pos)
	local start, finish = H.T.outer_escaped(win, pos)
	if start and finish and '"' ~= content:sub(start + 1, start + 1) then
		return start, finish
	end
end
...
local function pick_out(file, range, pos, action, copy)
	...
	if efinish1 and estart1 < range.start and efinish1 <= range.finish then
		table.insert(skips, efinish1)
	else
		estart1, efinish1 = nil, nil
	end
	local estart2, efinish2 = H.T.outer_string(vis.win, range.finish)
	if estart2 and estart2 < range.finish and estart2 >= range.start then
	...
	if #skips == 1 and not multisel then
		...
		if backward_splice and (backwards or visual) or forward_splice and (not backwards or visual) then
	...
	for i = #skips - 1, 1, -1 do
		...
		if len > 0 then
			...
			if esstart and esfinish and efstart and effinish and esstart == efstart then
				-- extend the kill range if it only partially covers a backslashed double quote:
				if '"' == file:content(esstart, 1) then
					if '\\"' == file:content(region.start - 1, 2) then
						region.start = region.start - 1
						range.start = range.start - (backwards and 1 or 0)
					end
					if '\\"' == file:content(region.finish - 1, 2) then
...
function H.O.repl_send(file, range, pos)
	...
	if not repl_fifo then
		repl_fifo = io.open(M.repl_fifo, "a+")
	end
...
local function spacekey(win, arg, selection)
	...
	local before_opening = d1:match(far_char) or far_char == '"' and ({H.T.outer_string(win, pos + 1)})[1] == pos + 1
	local next_opening = H.M.next_opening(win, pos)
...
local function electrify_any(win, arg, selection)
	...
	local far_char = win.file:content(pos + 1, 1)
	local after_space = prev_char and space:match(prev_char) or not prev_char
	local on_opening_quote =  next_char == '"' and ({H.T.outer_string(win, pos)})[1] == pos
	local on_opening_paren = d1:match(next_char)
	local after_opening = prev_char and d1:match(prev_char)
	local behind = pos - PREFIX_MAXLEN > 0 and pos - PREFIX_MAXLEN or 0
	...
	local after_closing = prev_char and (D2:match(prev_char) or prev_char == '"' and ({H.T.outer_string(win, pos - 1)})[2] == pos)
...
local function transpose()
	...
	if opening or closing then
		...
		local adjusted = vis.win.selections[opening or closing]
		get_sexps(vis.win, adjusted.pos)
		local fstart, ffinish = form_at(adjusted.pos + 1)
		if ffinish and ffinish > fstart + (opening and 2 or 0) then
			vis.win.file:delete(adjusted.range.finish, 1)
		end
		if form_len > 1 then
			vis.win.file:insert(adjusted.range.start, " ")
		end
		...
		if opening or closing then
			...
			local adjusted = vis.win.selections[opening or closing]
			if form_len > 1 then
				vis.win.file:insert(adjusted.range.finish, " ")
			end
			first_invalid = first.pos
			get_sexps(vis.win, last.pos)
			local fstart, ffinish = form_at(adjusted.pos + 1)
			if opening or closing and ffinish and (ffinish - fstart) > 2 then
				vis.win.file:delete(adjusted.range.start - 1, 1)
			end
		else
			vis:feedkeys("<vis-selections-rotate-left>")
	...
	for _, mode in pairs(vis.modes) do
		if action[mode] then
			if M.textobject[name] and M.autoselect then
				...
				win:map(mode, key, action[mode].binding, help)
			end
		end
	end
end
	...
	return function(win, pos)
		...
		if nestable then
			...
			if not vis.count and vis.mode == vis.modes.VISUAL or is_motion then
				local old = selection_by_pos(win, pos).range
			...
			for _ = 1, vis.count or 1 do
				...
				-- if we have reached the end of the parsed text, parse one or count paragraphs further and retry
				...
				-- TODO: reparsing with increasing count, as opposed to reparsing from incremented position,
				-- triggers an "avalanche" - it parses text that's already been parsed. Make the parsing "rolling" instead.
				local retry = 1
				while first_invalid > 0 and first_invalid < win.file.size and (not finish and start and pos == start or finish == first_invalid) do
					get_sexps(win, pos, retry)
