...
def --env dark-theme [] {
	dark set color_config
	$env.LS_COLORS = (vivid generate tokyonight-moon | str trim)
}

def --env light-theme [] {
	light set color_config
	$env.LS_COLORS = (vivid generate solarized-light | str trim)
}

def --env update-theme [] {
	# `do -i` to stop $env.LAST_EXIT_CODE from forwarding to shell
	# `try { } catch { }` because the read fails in light mode
	let theme = do -i { try { defaults read -g AppleInterfaceStyle err> /dev/null } catch { "Light" } };

	# return if the theme is already active
	if ($theme == ($env.CURRENT_THEME? | default "")) { return };
...
$env.config = {
    show_banner: "short" # true or false to enable or disable the welcome banner at startup
    ...
    hooks: {
        pre_prompt: [ # run before the prompt is shown
			# this is a string because a block of code does not
			# preserve $env.config updates (env vars *do* work).
			# https://www.nushell.sh/book/hooks.html#hooks-as-strings
			'(update-theme)'
		]
